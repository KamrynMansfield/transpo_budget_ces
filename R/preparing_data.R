...
decode_cells <- function(df, code_dictionary_df){
  ...
  def_codes_selected <- code_dictionary_df |>
    dplyr::select(Variable, value = `Code value`, description = `Code description`)
...
d_exp <- read_csv(expd_csv_path, guess_max = 10000)

# fmld.csv file
# This is the data with Consumer Unit Demographics and summary expenditures
# All four quarters have been combined into one file for the year
d_family <- read_csv(fmld_csv_path, guess_max = 10000)
...
def_codes <- read_excel(pumd_dictionary_xlsx_path, sheet = 3, guess_max = 100000)
def_variables_filtered <- filter_definition_data(def_variables, year = 2024) # get only they variables pertenent to 2023
def_codes_filtered <- filter_definition_data(def_codes, year = 2024) # get only they codes pertinent to 2023
...
d_family <- d_family |>
  ...
  dplyr::select(fam_col_defs$col_names_old) |>

  # filter so only one row per CU
  mutate(duplicate = duplicated(CUID)) |>
  filter(duplicate == FALSE)

# filter to just places in NYC area
# d_family <- d_family |>
#   filter(PSU == "S12A") |>
#   select(!c("PSU"))

# Convert all values to numeric (some were strings like "01", "02" etc.)
# WARNING. If the column isn't in numeric form, it will turn to NA
# for example, the PSU column all turns to NA
d_family[] <- lapply(d_family, as.numeric)

# Assume all the NAs are equal to zero
d_family[is.na(d_family)] <- 0
...
#              (`Car Maintenance` == 0 | is.na(`Car Maintenance`)) &
#              (`Car Purchase` == 0 | is.na(`Car Purchase`)) &
#              # (`Car Other` == 0 | is.na(`Car Other`)) &        # we got rid of this one by dispersing expenses into other categories
...
# turning all NAs to 0 (if there happen to be NAs)
